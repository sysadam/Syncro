...
if ($results.psobject.properties.value -contains $false) {
    ...
    Set-Asset-Field -Name "Windows 11 Ready" -Value $false
    Set-Asset-Field -Name "Windows 11 Results" -Value $($Results | Format-List | Out-String)
}
else {
    write-host "This device is compatible with Windows 11. See the detailed results for more information"
    Set-Asset-Field -Name "Windows 11 Ready" -Value $true
}